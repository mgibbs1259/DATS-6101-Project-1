...
Look at linearity and homoscedasticity
```{r}
...
ggplot(df_4, aes(x = total_expenditure, y = avg_score)) +
      ...
      ylab("Average Score")
#possible linearity and homoscedasticity
```
...
ggplot(df_4, aes(x = tr_sub_te, y = avg_score)) +
      ...
      ylab("Average Score")
#no linearity and possible homoscedasticity
```
...
summary(lm_df_4_te)
#residuals - not symmetric, model bias
#coefficients - intercept and slope have p values < 0.05, significant linear relationship between school expenditure and 4th grade average math scores
...
par(mfrow = c(2, 2))
plot(lm_df_4_te)
#linearity - the residuals vs fitted plot might have a fitted pattern and the red line is approximately horizontal at zero 
#normality - the normal Q-Q plot appears to approximately follow a straight line
#homoscedasticity - the scale-Location plot points are skewed to the left and the red line has a slight negative slope
```
...
summary(lm_df_8_te)
#residuals - not symmetric, model bias
#coefficients - intercept and slope have p values < 0.05, significant linear relationship between school expenditure and 8th grade average math scores
